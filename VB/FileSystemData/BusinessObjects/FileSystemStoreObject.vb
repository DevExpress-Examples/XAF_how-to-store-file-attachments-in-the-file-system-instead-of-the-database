...
Namespace FileSystemData.BusinessObjects
	...
	Public Class FileSystemStoreObject
		...
		Protected Overridable Sub SaveFileToStore()
			If Not String.IsNullOrEmpty(RealFileName) AndAlso TempSourceStream IsNot Nothing Then
				Try
...
#Region "IFileData Members"
		...
		Private Sub IFileData_LoadFromStream(ByVal fileName As String, ByVal source As Stream) Implements IFileData.LoadFromStream
			...
			If fileName <> Me.FileName Then ' updated, old code was: if (string.IsNullOrEmpty(tempFileName))
				tempFileName = RealFileName
			End If
			Me.FileName = fileName
			TempSourceStream = source
			Size = CInt(TempSourceStream.Length)
			OnChanged() 'T582918
		End Sub
		...
		End Sub
		<Persistent>
		Public Property Size() As Integer Implements IFileData.Size
			Get
